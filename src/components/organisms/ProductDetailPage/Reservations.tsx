...
const Reservations: FC<ReservationsProps> = ({ setShowReservationForm }) => {
  const router = useRouter();

  const { numPerson, timeRange, date, entity, sku } = router.query || {};

  const [departureDate, setDepartureDate] = useState<Date | null>(date ? new Date(date as string) : null);
  ...
  useEffect(() => {
    if (data) {
      ...
      if (status === Status.SUCCESS) {
        router.push(`/spa/reserve/${reservationData.orderId}`);
      }
  ...
  return (
    <>
      <Formik initialValues={initialState} onSubmit={handleSubmit} validationSchema={ReservationSchema}>
        {(props: FormikProps<Values>) => (
          <Form>
            <div className="bigLaptop:py-5 px-4">
              ...
              <div className="hidden bigLaptop:block">
                ...
                <FormikCustomInput
                  ...
                  inputClassName="placeholder:text-xs mobileBelow:ml-4 border-black bg-citiBlue-80"
                  name="timeRange"
                  placeholder={entity?.includes("vacation") ? "2 (in days)" : "2 (in hours)"}
                  type="number"
                />
              </div>
              <div className="hidden bigLaptop:block">
              ...
              <div className="hidden bigLaptop:block">
                <CustomButton
                  customClass="my-4 w-[90%] tablet:w-[80%] desktop:w-full"
                  handleClick={() => {}}
                  isDisabled={loading}
              ...
              <div className="fixed smallLaptop:hidden z-10 bottom-0 left-0 w-full bg-white py-2 px-4">
                <CustomButton
                  customClass="my-4 !w-full"
                  handleClick={handleMobileSubmit}
                  size={ButtonProperties.SIZES.big}
                  title="Reserve"
                  variant={ButtonProperties.VARIANT.secondary.name}
                />
